...
- `pow add` accepts multiple repos in one invocation: `pow add family-ties StudioOne web`. Partial failures continue and are reported in a summary; the command exits non-zero if any repo failed. Single-repo behavior (and exit codes) is unchanged.
